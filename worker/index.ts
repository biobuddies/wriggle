...
export default {
    async fetch(request: Request, env: Env): Promise<Response> {
        ...
        const request_length = encoder.encodeInto(
            request.url,
            new Uint8Array(worker.request.buffer),
        ).written
        ...
        const [headers, responseBody] = decoder
            .decode(new Uint8Array(http.buffer, offset, length))
            .split('\n\n')
